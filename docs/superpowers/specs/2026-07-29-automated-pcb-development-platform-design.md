...
> 项目级单一权威 EDA：既有 KiCad 9/10 项目，或通过 P0 能力门的嘉立创 EDA 专业版项目
...
1. 每个项目必须冻结唯一的权威 EDA、适配器 profile 和规则包摘要。既有 `kicad` 项目继续以 KiCad 为权威；只有 `lceda_pro` 项目通过对应 capability gate 后，嘉立创 EDA 专业版才能作为权威。任何项目都不得形成双主写入。
2. 确定性工作流内核是唯一有权推进阶段、修改工程和发布制品的组件。
3. AI 只能生成结构化提案、命令或审查结论，不能直接修改 EDA 原生文件、数据库状态或发布目录。
...
| 决策项 | 结论 | 原因 |
| --- | --- | --- |
| EDA 底座 | 项目级单一权威 EDA | 既有 KiCad 9/10 保持现有行为；`lceda_pro` 仅在公开、稳定、可回读的嘉立创专业版能力经过 P0 验证后启用 |
...
| EDA 交换 | 支持 KiCad 与嘉立创 EDA 专业版的隔离语义交换 | 每个项目只有一个权威工程，禁止双向自动合并；未验证的专有归档不得直接写入 |
...
- 不绕过权威 EDA 的 ERC/DRC、制造规则或人工审批门。
...
- 权威 EDA 的 ERC 无阻断错误；警告必须被修复或关联有效豁免。
...
CPL 至少包含引用标号、中心坐标、旋转、面和封装。适配器通过一组已知元件金样验证权威 EDA 的坐标系、底层镜像和旋转转换，不能依赖经验公式而无测试。

### 15.3 嘉立创 EDA 专业版交换

交换和适配器遵循以下边界：

- 每个项目在创建时冻结 `eda_kind`、`eda_profile_id`、`board_profile_id` 和 `rulepack_digest`；它们决定唯一允许写入的权威 EDA。
- `kicad` 项目继续以 KiCad 为权威，嘉立创专业版仅作为隔离交换与人工协作端。
- `lceda_pro` 项目仅在 `lceda-pro-v1` capability gate 证明存在公开、稳定、可回读的自动化入口后，才以嘉立创专业版为权威；KiCad 只用于 BoardIR 的语义与算法回归。
- 启动时记录软件版本、二进制摘要和可用交换能力；不直接修改无公开稳定规范的专有工程归档。
- 非权威 EDA 的导入必须进入隔离区，生成组件、网络、封装、板框和布局的语义差异；任何回写均通过明确的变更提案和对应权威 EDA 的验证。
- 能无歧义映射的变化生成 `exchange.import_change_proposal`；不支持或有损字段进入人工处理清单。
- 合并后重新执行权威 EDA 的 ERC、DRC、DFM 和相关审批门。

首版验收重点是“可追溯的隔离候选、受控导出和受控再导入”，不是无损实时协同编辑或双向自动合并。
...
| 双 EDA 内容漂移 | 版本冲突和丢失修改 | 项目级单一权威工程、隔离交换、语义差异和禁止自动双向合并 |
